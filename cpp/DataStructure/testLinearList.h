...
void testLinearList() {
    char temp = 0;
    int op = 1;
    char cur = 0, next = 0, pre = 0, elem = 0;
    int l_index = 0;
    int index;
    DS::linearList<char> lists[LEN];
    DS::Compare<char> comp = [](const char &a, const char &b) -> bool { return a == b; };
    DS::Visit<char> visit = [](const char &a) { std::cout << a << std::endl; };
    while (op != 0) {
        DS::cls();
        printf("\n\n");
        ...
        printf("    请选择你的操作[0~12]:");
        scanf("%d", &op);
        DS::linearList<char> &L = lists[l_index];
        switch (op) {
            case 1:
                printf("\nIntiaList功能！\n");
